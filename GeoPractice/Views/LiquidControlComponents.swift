...
/// One segment of a compact multi-option row (tab bar entries, hand/period
/// pickers). On iOS 26 this hands the button to the system's native Liquid
/// Glass button styles (`.glass` / `.glassProminent`) — the same rendering
/// pipeline behind stock controls like `Slider` — instead of hand-painting a
/// capsule with `.glassEffect()`, which only approximates it. Pre-iOS 26 it
/// falls back to the existing prototypeGlassSurface simulation.
struct PrototypeGlassSegment<Label: View>: View {
    let isSelected: Bool
    let action: () -> Void
        ...
        Button(action: action) {
            label()
                .background {
                    if isSelected {
                        Color.clear
                            .prototypeGlassSurface(cornerRadius: 999, emphasized: true)
                    }
                }
